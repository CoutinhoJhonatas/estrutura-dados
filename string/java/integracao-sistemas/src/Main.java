...
public class Main {
    ...
    public static String csvToJson(String[] videos) {
        ...
        for (int i = 0; i < videos.length; i++) {
            String[] parts = videos[i].split(",");

            sb.append("  {\n")
                    .append("    \"id\": \"").append(parts[0]).append("\",\n")
                    .append("    \"title\": \"").append(parts[1].substring(6).replaceAll(".mp4", "")).append("\",\n")
                    .append("    \"duration\": ").append(Integer.parseInt(parts[2])).append("\n")
                    .append(i == videos.length - 1 ? "  }\n" : "  },\n");
        }
